...
def resend_activation(request):
    ...
    if form.is_valid():
        ...
        try:
            ...
            if user.is_active:
                msg = 'Аккаунт с такой почтой уже активирован.'
                messages.info(request, msg)
                return redirect('users:login')
            send_activation_link(request, user, first=False)
            messages.info(
                request, 'Новая ссылка отправлена, проверьте свою почту.'
            )
            return redirect('users:login')
        except User.DoesNotExist:
            msg = 'Пожалуйста, укажите тот адрес, с которым регистрировались.'
...
def change_password(request):
    ...
    if form.is_valid():
        ...
        new = request.POST.get('new_password1')
        if old == new:
            messages.info(request, 'Вы не изменили свой пароль.')
        else:
            user = form.save()
